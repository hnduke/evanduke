...
def test_populate_database_no_faq_data_in_db_and_is_populated():
    ...
    assert FrequentlyAskedQuestion.objects.filter(service_type=ServiceType.FRACTIONAL_MANAGEMENT.value).count() == 7
    assert FrequentlyAskedQuestion.objects.filter(service_type=ServiceType.CONSULTATIONS.value).count() == 6
...
def test_populate_database_faq_data_already_in_db_so_is_not_populated(faqs):
    ...
    assert FrequentlyAskedQuestion.objects.filter(service_type=ServiceType.FRACTIONAL_MANAGEMENT.value).count() == 3
    assert FrequentlyAskedQuestion.objects.filter(service_type=ServiceType.CONSULTATIONS.value).count() == 3
    call_command("populate_database")
